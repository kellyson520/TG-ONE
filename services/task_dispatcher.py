...
class TaskDispatcher:
    ...
    async def start(self):
        ...
        logger.info(
            "🚀 TaskDispatcher 启动 "
            f"(Batch: {settings.TASK_DISPATCHER_BATCH_SIZE}, "
            f"QueueSize: {settings.WORKER_QUEUE_SIZE})"
        )
    ...
    async def stop(self):
        ...
        if self._task:
            ...
            try:
                ...
                logger.debug("TaskDispatcher 停止时取消后台任务")
    ...
    async def _dispatch_loop(self):
        ...
        while self.running:
            try:
                ...
                # 2. 批量拉取与原子锁定
                batch_size = min(
                    settings.TASK_DISPATCHER_BATCH_SIZE,
                    available_slots,
                )
                ...
                for task in tasks:
                    try:
                        ...
                        logger.error(
                            "Task pre-parse failed for ID %s: %s",
                            task.id,
                            e,
                        )
                        task.payload = {}
    ...
    async def _wait_for_queue_slot(self):
        ...
        if self._available_queue_slots() > 0:
            return
        ...
        try:
            ...
            logger.debug(
                "Dispatcher 背压等待超时: queue_size=%s, maxsize=%s",
                self.queue.qsize(),
                getattr(self.queue, "maxsize", 0),
            )
            return
    ...
    async def _adaptive_sleep(self):
        ...
        logger.debug(
            "Dispatcher 空载，进入休眠: %.2fs (Current: %.2fs)",
            sleep_time,
            self.current_sleep,
        )
        woke = False
        try:
            ...
            logger.debug(
                "Dispatcher 空载休眠超时: slept=%.2fs, current=%.2fs",
                sleep_time,
                self.current_sleep,
            )
        finally:
    ...
    def _available_queue_slots(self) -> int:
        """返回当前内存队列可用槽位，未设置 maxsize 时按批次上限处理。"""
    ...
    async def _prefetch_entities(self, tasks):
        ...
        for t in tasks:
            ...
            raw_task_data = getattr(t, "task_data", None)
            if not raw_task_data:
                continue

            try:
                # 尝试从 task_data 提取 chat_id
                data = json.loads(raw_task_data)
            except (TypeError, json.JSONDecodeError) as exc:
                logger.debug(
                    "跳过实体预热：task_data 无效 (task_id=%s): %s",
                    task_id,
                    exc,
                )
                continue
            ...
            if cid:
                ...
                logger.debug(
                    "跳过实体预热：缺少 chat_id/peer_id (task_id=%s)",
                    task_id,
                )
        ...
        logger.debug("Dispatcher 正在预热 %s 个实体缓存...", len(to_fetch))
        for cid in to_fetch:
            try:
    ...
    def get_stats(self):
        ...
        return {
            ...
            "throughput_per_min": (
                int(self.total_dispatched / (uptime / 60))
                if uptime > 60
                else 0
            ),
        }
