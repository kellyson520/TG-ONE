...
class ContentStrategy(BaseDedupStrategy):
    async def process(self, ctx: DedupContext) -> Optional[DedupResult]:
        ...
        if (
            is_video(message_obj)
            and not config.get("enable_content_hash_for_video", False)
            and not config.get("skip_media_sig", False)
        ):
        ...
        if await self._get_pcache_best_effort(ctx, pcache_key):
            try:
                ...
                ctx.logger.debug("内容哈希PCache指标记录失败: %s", exc)
            return DedupResult(
                True,
                "内容重复: persistent cache 命中",
                "content_hash",
                content_hash,
            )

        # 4. 检查数据库 (L3)
        is_dup, reason = await ctx.repo.check_content_hash_duplicate(
            content_hash,
            target_chat_id,
            config,
        )
        if is_dup:
            try:
                ...
                ctx.logger.debug("内容哈希指标记录失败: %s", exc)
