...
def clean_text_for_hash(text: str, strip_numbers: bool = False) -> str:
    ...
    # 3. CJK 处理: 给中日韩字符增加空格以便后续分词 (如果 SimHash 使用 split())
    # 或者直接在 SimHash 中处理。这里我们采用简单的转换。
...
def is_sticker(message_obj: Any) -> bool:
    """判断是否为表情包"""
    if hasattr(message_obj, 'sticker') and message_obj.sticker:
        return True
    return False

def extract_sticker_id(message_obj: Any) -> Optional[str]:
    """提取表情包唯一ID"""
    if hasattr(message_obj, 'sticker') and message_obj.sticker:
        # Telethon 使用 id
        return str(getattr(message_obj.sticker, 'id', ''))
    return None
