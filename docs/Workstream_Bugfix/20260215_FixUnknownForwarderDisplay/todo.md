...
### Phase 2: 后端逻辑修复
- [ ] 修改 `StatsRepository.get_rule_logs` 使用 `joinedload` 预加载 `rule`, `rule.source_chat` 和 `rule.target_chat`
- [ ] 修改 `StatsRepository.get_recent_activity` 同样预加载相关实体
- [ ] (可选) 检查 `RuleDTOMapper.log_to_dict` 的异常处理是否掩盖了关键错误

### Phase 3: 验证与验收
- [ ] 启动后端服务并检查 API 返回数据
- [ ] 在前端页面确认“来源实体”和“目标实体”已正确显示
- [ ] 归档任务报告
