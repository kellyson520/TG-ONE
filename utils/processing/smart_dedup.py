...
class SmartDeduplicator:
    ...
    async def check_duplicate(
        ...
        *,
        ...
        try:
            ...
            if signature:
                ...
                if is_dup:
                    ...
                    try:
                        ...
                        from utils.helpers.metrics import DEDUP_CHECK_SECONDS
            ...
            try:
                from utils.helpers.metrics import DEDUP_DECISIONS_TOTAL
            ...
            try:
                from utils.helpers.metrics import DEDUP_CHECK_SECONDS, DEDUP_DECISIONS_TOTAL
    ...
    async def _check_signature_duplicate(
        ...
        try:
            ...
            # 数据库检查（使用现有的MediaSignature表）
            logger.debug("开始数据库检查...")
            from utils.db.db_operations import DBOperations
            from utils.db.db_context import async_dedup_db_session

            logger.debug("获取数据库会话...")
            async with async_dedup_db_session() as session:
                logger.debug("初始化DBOperations...")
                db_ops = await DBOperations.create()
                logger.debug(
                    f"执行exists_media_signature查询，chat_id: {target_chat_id}, signature: {signature}"
                )
                exists = await db_ops.exists_media_signature(
                    session, str(target_chat_id), signature
                )
                logger.debug(f"数据库查询结果: {exists}")
                if exists:
                    logger.debug("数据库中存在匹配的签名")
                    return True, "数据库中存在"
            ...
            try:
                if config.get("time_window_hours", 24) <= 0:
                    ...
                    if bloom.probably_contains(
                        ...
                        from utils.helpers.metrics import DEDUP_HITS_TOTAL, DEDUP_QUERIES_TOTAL
    ...
    async def _check_similarity_duplicate(
        ...
        try:
                            ...
                            for ts_str in hits:
                                ...
                                if prev_text:
                                    ...
                                    if sim >= config.get("similarity_threshold", 0.85):
                                        try:
                                            from utils.helpers.metrics import DEDUP_FP_HITS_TOTAL
            ...
            try:
                from utils.helpers.metrics import DEDUP_SIMILARITY_COMPARISONS
    ...
    async def _check_video_duplicate_by_file_id(
        self, file_id: str, target_chat_id: int
    ) -> bool:
        """通过 telegram file id 检查视频是否重复（持久化层）"""
        try:
            from utils.db.db_operations import DBOperations
            from utils.db.db_context import async_dedup_db_session
            ...
            async with async_dedup_db_session() as session:
                db_ops = await DBOperations.create()
                return await db_ops.find_media_by_fileid_or_hash(
                    session, str(target_chat_id), file_id=file_id
                )
        except Exception:
            return False

    async def _check_video_duplicate_by_hash(
        self, vhash: str, target_chat_id: int
    ) -> bool:
        """通过内容哈希检查视频是否重复（持久化层）"""
        try:
            from utils.db.db_operations import DBOperations
            ...
            async with async_dedup_db_session() as session:
                db_ops = await DBOperations.create()
                return await db_ops.find_media_by_fileid_or_hash(
                    session, str(target_chat_id), content_hash=vhash
                )
        except Exception:
            return False
    ...
    async def _compute_video_partial_hash(
        ...
        try:
            if not getattr(message_obj, "media", None):
                ...
                return None
            from utils.helpers.metrics import VIDEO_PARTIAL_HASH_SECONDS
    ...
    async def _write_pcache(
        ...
        if not self.config.get("enable_persistent_cache", True):
            ...
            return
        try:
            from utils.db.persistent_cache import dumps_json, get_persistent_cache
    ...
    async def _flush_worker(self):
        ...
        while True:
            ...
            # 执行批量插入，使用asyncio.to_thread避免阻塞主事件循环
            try:
                from sqlalchemy import text

                from models.models import MediaSignature, get_dedup_session

                def _sync_write():
                    # 使用专门的批量插入 Session，不仅减少开销还能利用 SQLAlchemy Core 的高性能
                    with get_dedup_session() as session:
                        # 批量插入数据，处理 UNIQUE 约束冲突
                        from sqlalchemy.dialects.sqlite import insert as sqlite_insert
                        
                        stmt = sqlite_insert(MediaSignature.__table__).values(batch)
    ...
    async def _check_video_hash_pcache(self, file_id: str) -> Optional[str]:
        ...
        try:
            from utils.db.persistent_cache import get_persistent_cache, loads_json
