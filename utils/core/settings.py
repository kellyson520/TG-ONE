...
from utils.media.file_creator import AI_MODELS_CONFIG, create_default_configs
...
def load_ai_models(type="list"):
    ...
    try:
        models_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "config", "ai_models.json"
        )
        ...
        if _AI_MODELS_CACHE["data"] and (
            ...
            # 3. 更新缓存
            _AI_MODELS_CACHE["data"] = models_config
            _AI_MODELS_CACHE["ts"] = time.time()
...
def load_summary_times():
    ...
    try:
        times_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "config", "summary_times.txt"
        )
...
def load_delay_times():
    ...
    try:
        times_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "config", "delay_times.txt"
        )
        if not os.path.exists(times_path):
            create_default_configs()
...
def load_max_media_size():
    ...
    try:
        size_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "config", "max_media_size.txt"
        )
...
def load_media_extensions():
    ...
    try:
        size_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "config", "media_extensions.txt"
        )
        if not os.path.exists(size_path):
            create_default_configs()
