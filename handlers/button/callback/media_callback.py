...
async def handle_media_callback(event, **kwargs):
    ...
    try:
        ...
        # Note: Strategies currently handle session creation internally if needed for legacy compatibility.
        # We pass kwargs just in case.
...
async def callback_set_media_types(event, rule_id, session, message, data):
    ...
    try:
        async with container.db.get_session(session) as s:
            ...
            success, msg, media_types = await db_ops.get_media_types(s, rule.id)
...
async def callback_toggle_media_type(event, rule_id, session, message, data):
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            ...
            # 切换媒体类型状态
            db_ops = await get_db_ops()
            success, msg = await db_ops.toggle_media_type(s, rule.id, media_type)
                    ...
                    for sync_rule in sync_rules:
                        ...
                        try:
                            ...
                            target_success, _, target_media_types = (
                                await db_ops.get_media_types(s, sync_rule_id)
                            )
            ...
            # 重新获取媒体类型设置
            success, _, media_types = await db_ops.get_media_types(s, rule.id)
...
async def callback_set_media_extensions(event, rule_id, session, message, data):
    async with container.db.get_session(session) as s:
        await event.edit(
            "请选择要过滤的媒体扩展名：",
            buttons=await create_media_extensions_buttons(rule_id, page=0, session=s),
        )
    return


async def callback_media_extensions_page(event, rule_id, session, message, data):
    _, rule_id_str, page = data.split(":")
    rule_id = int(rule_id_str)
    page = int(page)
    async with container.db.get_session(session) as s:
        await event.edit(
            "请选择要过滤的媒体扩展名：",
            buttons=await create_media_extensions_buttons(rule_id, page=page, session=s),
        )
    return
...
async def callback_toggle_media_extension(event, rule_id, session, message, data):
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            ...
            # 获取当前规则已选择的扩展名
            db_ops = await get_db_ops()
            selected_extensions_data = await db_ops.get_media_extensions(s, rule.id)
            selected_extension_list = [ext["extension"] for ext in selected_extensions_data]
            ...
            if was_selected:
                ...
                extension_id = next(
                    (
                        ext["id"]
                        for ext in selected_extensions_data
                                    ...
                                    if extension in target_extension_list:
                                        ...
                                        if target_extension_id:
                                            await db_ops.delete_media_extensions(
                                                s, sync_rule_id, [target_extension_id]
                                            )
                                            logger.info(
            ...
            await event.edit(
                "请选择要过滤的媒体扩展名：",
                buttons=await create_media_extensions_buttons(rule_id, page=current_page, session=s),
            )
