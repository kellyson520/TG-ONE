    ...
    try:
        async with container.db.get_session(session) as s:
            ...
            if not rule:
                ...
                return
            await message.edit("请选择要同步到的规则：", buttons=await create_sync_rule_buttons(rule_id, page=0, session=s))
...
async def callback_toggle_rule_sync(event, rule_id_data, session, message, data):
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            ...
            await message.edit("请选择要同步到的规则：", buttons=await create_sync_rule_buttons(source_rule_id, page, session=s))
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            ...
            if not rule:
                await event.answer("规则不存在")
                return
            await message.edit("请选择要同步到的规则：", buttons=await create_sync_rule_buttons(rule_id, page, session=s))
    except Exception as e:
