...
async def cancel_state_after_timeout(
    ...
    # 使用 session_manager 检查并清除状态
    if user_id in session_manager.user_sessions:
        if chat_id in session_manager.user_sessions[user_id]:
            current_state = session_manager.user_sessions[user_id][chat_id].get("state")
            if current_state:
                logger.info(
                    f"状态超时自动取消 - user_id: {user_id}, chat_id: {chat_id}, state: {current_state}"
                )
                del session_manager.user_sessions[user_id][chat_id]
