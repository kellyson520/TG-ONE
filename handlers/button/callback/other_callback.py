...
async def handle_other_callback(event, **kwargs):
    """处理通用规则设置回调 (异步版) - Refactored to use Strategy Registry with fallback"""
    ...
    # Try dispatching to Strategy Registry first
    ...
    async with container.db.get_session(kwargs.get("session")) as session:
        ...
        if handler:
            ...
            logger.warning(f"OtherCallback: No legacy handler found for {action}")
...
async def callback_other_settings(event, rule_id, session, message, data):
    await event.edit(
        "其他设置：", buttons=await create_other_settings_buttons(rule_id=rule_id, session=session)
    )
    return
    ...
    try:
        async with container.db.get_session(session) as s:
            ...
            dup_list, dup_map = await db_ops.scan_duplicate_media(
                s, rule.target_chat.telegram_chat_id
            )
            if not dup_list:
...
async def callback_view_source_messages(event, rule_id, session, message, data):
    ...
    try:
        async with container.db.get_session(session) as s:
            ...
            db_ops = await DBOperations.create()
            # 获取完整记录
            records = await db_ops.get_duplicate_media_records(s, rule.target_chat.telegram_chat_id)
...
async def callback_confirm_delete_duplicates(event, rule_id, session, message, data):
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            ...
            dup_list, dup_map = await db_ops.scan_duplicate_media(s, target_chat_id_str)
...
async def callback_handle_ufb_item(event, rule_id, session, message, data):
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            ...
            source_chat = find_chat_by_telegram_id_variants(s, chat.current_add_id)
            if not source_chat:
...
async def callback_toggle_allow_delete_source_on_dedup(
    ...
    try:
        async with container.db.get_session(session) as s:
            ...
            # 返回上一页
            await callback_dedup_scan_now(event, rule_id, s, message, data)
    except Exception as e:
        logger.error(f"切换删除源消息权限失败: {str(e)}")
        await event.answer("操作失败，请检查日志")


async def create_copy_rule_buttons(rule_id, page=0, session=None):
    """创建复制规则按钮列表
    ...
    async with container.db.get_session(session) as s:
        ...
        current_rule = await s.get(ForwardRule, source_rule_id)
        if not current_rule:
...
async def callback_perform_copy_rule(event, rule_id_data, session, message, data):
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            # 获取源规则和目标规则
            source_rule = await s.get(ForwardRule, source_rule_id)
            target_rule = await s.get(ForwardRule, target_rule_id)
            ...
            if hasattr(source_rule, "media_types") and source_rule.media_types:
                from sqlalchemy import select
                stmt = select(MediaTypes).filter_by(rule_id=target_rule.id)
                res = await s.execute(stmt)
                target_media_types = res.scalar_one_or_none()
            ...
            if hasattr(source_rule, "rule_syncs") and source_rule.rule_syncs:
                for sync in source_rule.rule_syncs:
                    # 检查是否已存在
                    exists = any(
                        sy.sync_rule_id == sync.sync_rule_id for sy in target_rule.rule_syncs
                    )
                    if not exists:
                        # 确保不会创建自引用的同步关系
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            ...
            source_rule, target_rule = await get_rules(
                event, s, source_rule_id, target_rule_id
            )
            if not source_rule or not target_rule:
                return
...
# 通用辅助函数
async def show_rule_selection(event, rule_id, data, title, callback_action, session=None):
    """显示规则选择界面的通用函数
    ...
    # 创建规则选择按钮
    buttons = await create_rule_selection_buttons(
        source_rule_id, page, callback_action, session
    )
    await event.edit(title, buttons=buttons)


async def create_rule_selection_buttons(
    rule_id, page=0, callback_action="perform_copy_rule", session=None
):
    """创建规则选择按钮的通用函数
...
async def callback_clear_keyword(event, rule_id, session, message, data):
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            # 获取规则信息
            current_rule = await s.get(ForwardRule, int(rule_id))
            if not current_rule:
                await event.answer("规则不存在")
                return
            ...
            # 检查是否有其他规则
            stmt = select(func.count(ForwardRule.id)).filter(ForwardRule.id != current_rule.id)
            result = await s.execute(stmt)
            other_rules_count = result.scalar() or 0
            ...
            if other_rules_count > 0:
                ...
                other_buttons = await create_rule_selection_buttons(
                    rule_id, page, "perform_clear_keyword", s
                )
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            ...
            if other_rules_count > 0:
                ...
                # 添加其他规则按钮
                other_buttons = await create_rule_selection_buttons(
                    rule_id, page, "perform_clear_replace", s
                )
...
async def callback_delete_rule(event, rule_id, session, message, data):
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            # 获取规则信息
            current_rule = await s.get(ForwardRule, int(source_rule_id))
            if not current_rule:
                await event.answer("规则不存在")
                return
            ...
            if other_rules_count > 0:
                ...
                # 添加其他规则按钮
                other_buttons = await create_rule_selection_buttons(
                    rule_id, page, "perform_delete_rule", s
                )
...
async def callback_perform_clear_keyword(event, rule_id_data, session, message, data):
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            ...
            await s.execute(delete(Keyword).filter(Keyword.rule_id == rule.id))
            await s.commit()
    ...
    try:
        ...
        async with container.db.get_session(session) as s:
            ...
            if hasattr(rule, "rule_syncs"):
                await s.execute(delete(RuleSync).filter(RuleSync.rule_id == rule.id))
                await s.execute(
            ...
            deleted_chats = await check_and_clean_chats(s, rule_obj)
            if deleted_chats > 0:
        ...
        # Note: s might not be bound if get_session failed
