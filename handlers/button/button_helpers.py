...
async def create_media_settings_buttons(rule=None, rule_id=None):
    ...
    for field, config in MEDIA_SETTINGS.items():
        ...
        # 处理高级媒体筛选选项
            ...
            buttons.append([Button.inline(button_text, callback_data)])
            continue
        elif field in ["duration_range", "resolution_range", "file_size_range"]:
            # 显示当前设置的范围
            if field == "duration_range":
                min_val = getattr(rule, "min_duration", 0)
                max_val = getattr(rule, "max_duration", 0)
                range_text = f"{min_val}s - {max_val if max_val > 0 else '∞'}s"
            elif field == "resolution_range":
                min_w = getattr(rule, "min_width", 0)
                max_w = getattr(rule, "max_width", 0)
                min_h = getattr(rule, "min_height", 0)
                max_h = getattr(rule, "max_height", 0)
                range_text = f"{min_w}x{min_h} - {max_w if max_w > 0 else '∞'}x{max_h if max_h > 0 else '∞'}"
            else:  # file_size_range
                min_size = getattr(rule, "min_file_size", 0)
                max_size = getattr(rule, "max_file_size", 0)
                range_text = f"{min_size}KB - {max_size if max_size > 0 else '∞'}KB"

            button_text = f"{config['display_name']}: {range_text}"
            callback_data = f"{config['toggle_action']}:{rule.id}"
            buttons.append([Button.inline(button_text, callback_data)])
            continue
        else:
...
async def create_list_buttons(total_pages, current_page, command):
    ...
    # 上一页按钮
    if current_page > 1:
        row.append(Button.inline("⬅️ 上一页", f"page:{current_page-1}:{command}"))

    # 页码显示
    row.append(Button.inline(f"{current_page}/{total_pages}", "noop:0"))  # 空操作

    # 下一页按钮
    if current_page < total_pages:
        row.append(Button.inline("下一页 ➡️", f"page:{current_page+1}:{command}"))
...
async def create_summary_time_buttons(rule_id, page=0):
    ...
    for i, time in enumerate(SUMMARY_TIMES[start_idx:end_idx], start=1):
        current_row.append(Button.inline(time, f"select_time:{rule_id}:{time}"))
    ...
    nav_buttons = []
    if page > 0:
        nav_buttons.append(Button.inline("⬅️ 上一页", f"time_page:{rule_id}:{page - 1}"))
    ...
    if end_idx < total_times:
        nav_buttons.append(Button.inline("下一页 ➡️", f"time_page:{rule_id}:{page + 1}"))
...
async def create_media_size_buttons(rule_id, page=0):
    ...
    for i, size in enumerate(MEDIA_SIZE[start_idx:end_idx], start=1):
        current_row.append(
            Button.inline(str(size), f"select_max_media_size:{rule_id}:{size}")
        )
    ...
    if page > 0:
        nav_buttons.append(
            Button.inline("⬅️ 上一页", f"media_size_page:{rule_id}:{page - 1}")
        )
    ...
    if end_idx < total_size:
        nav_buttons.append(
            Button.inline("下一页 ➡️", f"media_size_page:{rule_id}:{page + 1}")
        )
...
async def create_delay_time_buttons(rule_id, page=0):
    ...
    for i, time in enumerate(DELAY_TIMES[start_idx:end_idx], start=1):
        current_row.append(
            Button.inline(str(time), f"select_delay_time:{rule_id}:{time}")
        )
    ...
    if page > 0:
        nav_buttons.append(
            Button.inline("⬅️ 上一页", f"delay_time_page:{rule_id}:{page - 1}")
        )
    ...
    if end_idx < total_times:
        nav_buttons.append(
            Button.inline("下一页 ➡️", f"delay_time_page:{rule_id}:{page + 1}")
        )
...
async def create_media_extensions_buttons(rule_id, page=0):
    ...
    try:
        ...
        page_buttons = []
        total_pages = (
            total_extensions + extensions_per_page - 1
        ) // extensions_per_page
        ...
        if total_pages > 1:
            ...
            if page > 0:
                page_buttons.append(
                    Button.inline("⬅️", f"media_extensions_page:{rule_id}:{page-1}")
                )
            else:
...
async def create_sync_rule_buttons(rule_id, page=0):
    ...
    try:
        ...
        if total_pages > 1:
            ...
            if page > 0:
                page_buttons.append(
                    Button.inline("⬅️", f"sync_rule_page:{rule_id}:{page-1}")
                )
            else:
...
async def create_push_settings_buttons(rule_id, page=0):
    ...
    try:
        ...
        for config in push_configs[start_idx:end_idx]:
            ...
            # 创建按钮
            buttons.append(
                [Button.inline(button_text, f"toggle_push_config:{config.id}")]
            )
