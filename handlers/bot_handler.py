...
async def _resolve_command_context(event, trace_id):
    """解析命令上下文：验证管理员、获取chat/bot信息"""
    ...
    bot_user_id = int(await get_user_id())
    chat_id = chat.id

    return {"message": message, "user_id": user_id, "chat_id": chat_id, "bot_user_id": bot_user_id}


async def _handle_non_command_input(event, client, user_id, chat_id, trace_id):
    """处理非命令输入：链接转发或提示词状态"""
    message = event.message

    if not message.text.startswith("/") and chat_id == int(await get_user_id()):
    ...
    if not message.text.startswith("/"):
        ...
        if chat_state_data and chat_state_data.get('state'):
            ...
            res = await handle_prompt_setting(event, client, user_id, chat_id, chat_state_data['state'], event)
            if res:
...
def _build_command_handlers(event, client, parts, command):
    """构建命令分发表"""
    return {
        ...
        "menu": lambda: handle_settings_command(event, "menu", parts),
...
async def _handle_flood_wait(e, event, trace_id):
    """处理FloodWait速率限制异常"""
    ...
    try:
        ...
        logger.warning(
            f"⚠️ [Bot命令] FloodWait恢复提示发送失败: TraceID={trace_id}, 错误={notify_e}"
        )
...
async def handle_command(client, event):
    ...
    try:
        ctx = await _resolve_command_context(event, trace_id)
        if not ctx:
            return

        message, user_id, chat_id, bot_user_id = ctx["message"], ctx["user_id"], ctx["chat_id"], ctx["bot_user_id"]

        if await _handle_non_command_input(event, client, user_id, chat_id, trace_id):
            return
        ...
        command_handlers = _build_command_handlers(event, client, parts, command)
        ...
        await _handle_flood_wait(e, event, trace_id)
