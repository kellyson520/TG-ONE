...
# 1. 模块级缓存
_CACHED_MAX_SIZE: Optional[float] = None


def _load_max_size() -> float:
    """加载并解析MAX_MEDIA_SIZE配置"""
    try:
        from core.config import settings
        return settings.MAX_MEDIA_SIZE * 1024 * 1024
    except Exception as e:
        logger.error(f"MAX_MEDIA_SIZE 解析失败: {e}，使用默认值")
        return 2000.0 * 1024 * 1024


async def get_media_size(media: Any) -> int:
    """获取媒体文件大小"""
    if not media:
        return 0
    ...
    try:
        ...
        if hasattr(media, "document") and media.document:
            return int(media.document.size)
        ...
        if hasattr(media, "photo") and media.photo:
            ...
            return int(largest_photo.size) if hasattr(largest_photo, "size") else 0
...
async def get_max_media_size() -> float:
    """获取媒体文件大小上限（带缓存）"""
...
async def download_media_with_retry(client: Any, media: Any, file: Any, retries: int = 3) -> Any:
    """带重试机制的媒体下载"""
    import asyncio
