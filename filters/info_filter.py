...
class InfoFilter(BaseFilter):
    ...
    async def _process(self, context):
        """
        添加原始链接和发送者信息，并提供可用于后续过滤的 sender_id 等上下文变量
        ...
        try:
            ...
            # 添加发送者信息（并在 context 上保存 sender_id/sender_name 供下游过滤器使用）
                    ...
                    if hasattr(event.message, 'sender_chat') and event.message.sender_chat:
                        ...
                        if hasattr(peer, 'channel_id'):
                            ...
                            try:
                                ...
                                users_info = await batch_service.get_users_info([sender_id])
                                
                                if str(sender_id) in users_info:
                                    user_data = users_info[str(sender_id)]
                                    sender_name = (
                                    ...
                                    if entity_resolver:
                                        channel = await entity_resolver.resolve_single_entity(peer.channel_id)
                                        sender_name = channel.title if channel and hasattr(channel, 'title') else "Unknown Channel"
                                    else:
                                        # 如果所有优化方法都不可用，使用默认名称
                                        sender_name = "Unknown Channel"
                                        logger.warning("实体解析器未初始化，使用默认频道名称")
