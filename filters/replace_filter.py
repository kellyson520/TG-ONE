...
class ReplaceFilter(BaseFilter):
    ...
    async def _process(self, context):
        ...
        try:
            ...
            replace_rules = []
            try:
                if hasattr(rule, 'replace_rules') and rule.replace_rules:
                    replace_rules = rule.replace_rules
            except Exception as e:
                logger.debug(f"获取替换规则失败: {str(e)}")
                return True  # 无法获取替换规则，跳过替换但继续处理
            
            if not replace_rules:
                return True  # 没有替换规则，直接返回
            ...
            # [Scheme 7 Fix] 将修改后的文本写入metadata，供SenderMiddleware使用
            context.metadata[f'modified_text_{rule.id}'] = message_text
            context.metadata['modified_text'] = message_text
