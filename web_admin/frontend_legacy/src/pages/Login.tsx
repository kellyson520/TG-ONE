    ...
    try {
      if (step === 'credentials') {
        ...
        if (res.success) {
          ...
          addNotification({ message: res.error || '登录失败', type: 'error' });
        }
      } else {
        const res: any = await authService.login2fa({
        ...
        if (res.success) {
          ...
          addNotification({ message: res.error || '验证码错误', type: 'error' });
        }
      }
    } catch (error: any) {
      const errorMsg = error.response?.data?.detail || error.response?.data?.error || '登录请求失败';
