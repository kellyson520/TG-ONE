...
// Mock data for charts - will be replaced with real data soon
const trafficData = [
  { name: '周一', value: 12000 },
  { name: '周二', value: 15234 },
  { name: '周三', value: 18900 },
...
export function Dashboard() {
  ...
  const fetchData = async () => {
    try {
      const overviewRes = await statsService.getOverview();
      if (overviewRes.success) {
        const { data } = overviewRes;
        setStats({
          todayForwards: data.forward_stats?.total_forwards || 0,
          activeRules: data.overview?.active_rules || 0,
          dedupCache: data.dedup_stats?.cached_signatures || 0,
          errorRate: data.queue_status?.error_rate || 0,
        });
      }

      const resourcesRes = await statsService.getSystemResources();
      if (resourcesRes.success) {
        const { data } = resourcesRes;
        setResources({
          cpu: data.cpu_percent || 0,
          memory: data.memory_percent || 0,
          dbSize: data.db_size_mb || 0,
        });
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((card, index) => (
          ...
          >
            <div
              className={`absolute top-0 left-0 w-1 h-full ${card.color === 'primary'
                  ? 'bg-primary'
                  : card.color === 'success'
                    ? 'bg-emerald-500'
                    : card.color === 'warning'
            ...
            <CardContent className="p-6">
              <div className="flex items-start justify-between">
                ...
                <div
                  className={`w-10 h-10 rounded-lg flex items-center justify-center ${card.color === 'primary'
          ...
          <CardContent>
            <div className="space-y-3 max-h-[200px] overflow-auto">
              {activityLogs.map((log) => (
                ...
                >
                  <div
                    className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${log.type === 'success'
                        ? 'bg-emerald-500'
                        : log.type === 'warning'
                          ? 'bg-amber-500'
                          : 'bg-sky-500'
