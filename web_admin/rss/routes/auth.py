    ...
    try:
        ...
        if not users:
            return RedirectResponse(
                url="/rss/register", status_code=status.HTTP_302_FOUND
            )
...
async def login(
    ...
    try:
        ...
        response = RedirectResponse(
            url="/rss/dashboard", status_code=status.HTTP_302_FOUND
        )
        response.set_cookie(
...
async def logout():
    # 登出也重定向到RSS 登录页
    response = RedirectResponse(url="/rss/login", status_code=status.HTTP_302_FOUND)
...
async def change_password(
    ...
    try:
        ...
        if len(new_password) < 8:
            return JSONResponse(
                {"success": False, "message": "新密码长度必须至少为8个字符"}
            )
        ...
        with get_session() as db_session:
            init_db_ops()
            is_valid = await db_ops.verify_user(
                db_session, user.username, current_password
            )
            if not is_valid:
                return JSONResponse({"success": False, "message": "当前密码不正确"})
            # 更新密码
            success = await db_ops.update_user_password(
                db_session, user.username, new_password
            )
            if not success:
                return JSONResponse(
                    {"success": False, "message": "修改密码失败，请重试"}
                )
