...
async def get_feed(rule_id: int, request: Request):
    ...
    try:
        ...
        if not rss_config or not rss_config.enable_rss:
            logger.warning(f"规则 {rule_id} 的RSS未启用或不存�?)
            raise HTTPException(status_code=404, detail="RSS feed 未启用或不存�?)
        ...
        logger.info(f"获取�?{len(entries)} 个条�?)
        
        # 如果没有条目，返回测试数�?
        if not entries:
            logger.warning(f"规则 {rule_id} 没有条目数据，返回测试数�?)
            try:
                ...
                if isinstance(rss_xml, bytes):
                    logger.info("将RSS XML从字节转换为字符�?)
                    rss_xml = rss_xml.decode('utf-8')
                
                # 记录XML内容的一部分
                xml_sample = rss_xml[:500] + "..." if len(rss_xml) > 500 else rss_xml
                logger.info(f"生成的测试RSS XML (�?00字符): {xml_sample}")
                
                # 检查XML中是否还有硬编码的localhost�?27.0.0.1地址
                ...
                logger.error(f"生成测试Feed时出�? {str(e)}", exc_info=True)
            ...
            try:
                ...
                # 记录XML内容的一部分
                xml_sample = rss_xml[:500] + "..." if len(rss_xml) > 500 else rss_xml
                logger.info(f"生成的RSS XML (�?00字符): {xml_sample}")
                ...
                logger.error(f"生成真实条目Feed时出�? {str(e)}", exc_info=True)
                raise HTTPException(status_code=500, detail=f"生成Feed失败: {str(e)}")
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"生成RSS feed时出�? {str(e)}", exc_info=True)
...
async def get_media(rule_id: int, filename: str, request: Request):
    ...
    if not media_path.exists():
        # 文件不存在，返回404
        logger.error(f"媒体文件未找�? {filename}")
        raise HTTPException(status_code=404, detail=f"媒体文件未找�? {filename}")
    ...
    logger.info(f"发送媒体文�? {filename}, MIME类型: {mime_type}, 大小: {os.path.getsize(media_path)} 字节")
...
async def add_entry(rule_id: int, entry_data: Dict[str, Any] = Body(...)):
    ...
    try:
        ...
        logger.info(f"接收到新条目数据: 规则ID={rule_id}, 标题='{entry_data.get('title', '无标�?)}', 媒体数量={media_count}, 包含上下�?{has_context}")
        ...
        # 记录上下文信�?
        if has_context:
            logger.info(f"条目包含原始上下文对象，属�? {', '.join(entry_data['context'].keys()) if hasattr(entry_data['context'], 'keys') else '无法获取属�?}")
        ...
        if len(current_entries) >= max_items - 1:
            # 计算需要删除的条目数量，确保添加新条目后总数不超过最大限�?
            to_delete_count = len(current_entries) - (max_items - 1)
            if to_delete_count > 0:
                logger.info(f"当前条目数量({len(current_entries)})将超过限�?{max_items})，需要删�?{to_delete_count} 个最早的条目")
                ...
                for entry in entries_to_delete:
                    try:
                        ...
                        if hasattr(entry, 'media') and entry.media:
                            logger.info(f"条目 {entry.id} 包含 {len(entry.media)} 个媒体文件，将一并删�?)
                                ...
                                if hasattr(media, 'filename'):
                                    ...
                                    if media_path.exists():
                                        try:
                                            ...
                                            logger.info(f"已删除媒体文�? {media_path}")
                                        except Exception as e:
        ...
        logger.info(f"启用自定义标题模�? {rss_config.enable_custom_title_pattern}, 启用自定义内容模�? {rss_config.enable_custom_content_pattern}")
        if rss_config.enable_custom_title_pattern or rss_config.enable_custom_content_pattern:
            try:
                    ...
                    for i, pattern in enumerate(content_patterns):
                        try:
                            ...
                            if match and match.groups():
                                ...
                                processing_content = extracted_content  # 更新处理内容为提取结�?
                ...
                if not entry.title and original_title:
                    ...
                    logger.info(f"恢复原标�? {entry.title}")
        ...
        if entry.original_link:
            ...
            if clean_link.startswith("http"):
                if entry.author:
                    ...
                    # 使用Markdown格式的链�?
                    entry.content += f'\n\n[来源]({clean_link})'
                logger.info(f"已添加清理后的链�?Markdown格式): {clean_link}")
            else:
...
async def delete_entry_api(rule_id: int, entry_id: str):
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=404, detail="条目未找�?)
        
        return {"status": "success", "message": "条目已删�?}
    except Exception as e:
...
async def delete_rule_data(rule_id: int):
    ...
    try:
        ...
        # 删除媒体目录
        if media_path.exists():
            logger.info(f"开始删除媒体目�? {media_path}")
            success, method = force_delete_directory(media_path)
            if success:
                deleted_dirs += 1
                logger.info(f"已删除媒体目�? {media_path} - {method}")
            else:
        ...
        # 删除数据目录
        if data_path.exists():
            logger.info(f"开始删除数据目�? {data_path}")
            success, method = force_delete_directory(data_path)
            if success:
                deleted_dirs += 1
                logger.info(f"已删除数据目�? {data_path} - {method}")
            else:
        ...
        return {
            "status": status,
            "message": f"处理规则 {rule_id} 的数据{'失败，目录仍然存�? if remaining_paths else '成功，目录已删除'}",
            "details": {
        ...
        logger.error(f"删除规则数据时出�? {str(e)}")
        raise HTTPException(status_code=500, detail=f"删除规则数据时出�? {str(e)}") 